...
export class ActivitiesCreate extends React.Component<props.parentsItem, {}> {
	...
	send(event:any){
		...
		ajax({
			method:"POST",
	        url: `${window.settings.uri}/v1/activities/:grade/:course/?PublicKeyId=:publickeyId&PrivateKeyId=:privateKeyId}`,
	        dataType: "json",
	        data:data,	        
	        success:(data:any)=>{
	        	this.props.router.replace('/activities');
	        },
	        error:(data:any)=>{
    ...
    return (
    	<div className="container">    				
    		<form method="POST" className="formulario" onSubmit={(e)=>{this.send(e)}}>
				  ...
				  <div className="form-group">
				    <label htmlFor="expiration"><b>Expiracion*</b></label>
				    <input type="date" className="form-control" id="expiration" aria-describedby="emailHelp" placeholder="YYYY-mm-dd" onChange={(e)=>{this.getFields(e)}}/>
				    {this.state.error.expiration && (
				    	<div className="alert alert-danger" role="alert">
