...
export class Item extends React.Component<props.gradeItem, {}> {
	deleteField(event: any){
		ajax({
			method:"DELETE",
	        url: `//localhost:8080/v1/grade/${this.props.grade._id}?PublicKeyId=${this.props.session.publicKeyId}&PrivateKeyId=${this.props.session.privateKeyId}`,
	        dataType: "json",
	        data:null,
	        crossDomain:true,
	        success:(data:any)=>{
			...
			<div className="item">
				<img src="/images/user-login-icon-14.png" alt="Perfil de usuario" className="rounded-circle" width="84" height="84"/>
	  			<div className="container-element text-align center">
	  				<Link to={`/grade/${this.props.grade._id}`} className="title">{this.props.grade.data.name}</Link>
