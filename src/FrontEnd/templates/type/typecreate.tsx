...
@withRouter
export class TypeCreate extends React.Component<{}, {}> {
	...
	send(event:any){
		...
		ajax({
			method:"POST",
	        url: `${window.settings.uri}/v1/learning/type/?PublicKeyId=${this.session.publicKeyId}&PrivateKeyId=${this.session.privateKeyId}`,
	        dataType: "json",
	        data:data,	        
	        success:(data:any)=>{
	        	this.props.router.replace('/type');
	        },
	        error:(data:any)=>{
