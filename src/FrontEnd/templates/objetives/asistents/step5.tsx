...
export class Step5 extends React.Component<Props.GenericRouter, states.SetCognitions> {
  ...
  public state:states.SetCognitions={
	...
	componentDidMount(){
		ajax({
			method:"GET",
	        url: `${window.settings.uri}/v1/helps/objetives/${this.props.routeParams.objetive}/cognitions?PublicKeyId=${this.session.publicKeyId}&PrivateKeyId=${this.session.privateKeyId}`,
	        dataType: "json",
	        data:null,
	        success:(data:Array<crud.cognition>)=>{
						this.loadding=true;
						if(data.length==0){
							this.props.router.replace(`/error/resolve/objetive/${this.props.routeParams.objetive}/cognition/step1`);
							return;
						}
				    this.setState({
				      cognitions : data
