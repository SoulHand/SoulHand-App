...
describe("Test route static height",function(){
	...
	it("GET /v1/physic/static/height",function(done){
		...
		find.save().then(function(){			
			return utils.runApp("GET","/v1/physic/static/height/");
	...
	it("POST /v1/physic/static/height",function(done){
		...
		var name=10;
		find.save().then(function(){			
			return utils.runApp("POST","/v1/physic/static/height/?PublicKeyId="+user.publicKeyId+"&PrivateKeyId="+user.privateKeyId,{
				form:{
					age:name,
					min:15,
					max:25
	...
	it("GET /v1/physic/static/height/:id",function(done){
		...
		find.save().then(function(){
			return utils.runApp("GET","/v1/physic/static/height/"+find._id);
		}).then(function(response){
	...
	it("PUT /v1/physic/static/height/:id",function(done){
		...
		find.save().then(function(){
			return utils.runApp("PUT","/v1/physic/static/height/"+find._id+"?PublicKeyId="+user.publicKeyId+"&PrivateKeyId="+user.privateKeyId,{
			form:{
				age:25,
				min:100,
				max:180	
	...
	it("DELETE /v1/physic/static/height/:id",function(done){
		...
		find.save().then(function(){
			return utils.runApp("DEL","/v1/physic/static/height/"+find._id+"?PublicKeyId="+user.publicKeyId+"&PrivateKeyId="+user.privateKeyId);
		}).then(function(response){
